...
2. **Manager / Atasan**: Atasan langsung dari Staff. Bertugas menetapkan KPI spesifik ke Staff, me-review logbook harian, dan memberikan *Rating* (1-5).
...
Untuk mencegah perubahan data yang tidak sah (misal: mengedit foto setelah dinilai) dan menangani kesalahan *user* (misal: tidak sengaja klik selesai), Logbook menggunakan sistem *State Machine* dengan 3 status utama:
...
*   **`SUBMITTED` (Menunggu Review)**
    ...
    *   *(Opsional: Manager dapat menekan tombol "Revert/Kembalikan" mengubah status kembali ke `DRAFT` jika bukti kurang).*
*   **`REVIEWED` (Selesai & Dinilai)**
    *   **Pemicu:** Berubah saat Manager memberikan *Rating* (1-5) dan menekan "Simpan Penilaian".
...
| `PUT` | `/auth/change-password` | All | User ubah password mereka sendiri. Validasi *old password* & update *new password*. |
...
*State Machine* (`DRAFT` -> `SUBMITTED` -> `REVIEWED`) diimplementasikan di sini.
| Method | Endpoint | Roles | Description |
| :--- | :--- | :--- | :--- |
...
Endpoint yang dioptimasi (dengan *Redis Cache*) untuk UI Dasbor.
| Method | Endpoint | Roles | Description |
| :--- | :--- | :--- | :--- |
