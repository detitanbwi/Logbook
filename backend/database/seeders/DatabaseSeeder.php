...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $this->command?->info('Seeded users: exactly 4 (SuperAdmin, Admin, 2 Staff hierarchy)');
        ...
        $kpis = collect([
            ...
            KpiMaster::factory()->create([
                ...
                'target_angka' => 6,
                'satuan' => 'tiket',
                'deskripsi' => 'Koordinasi penyelesaian tindak lanjut dari temuan lapangan.',
            ]),
            KpiMaster::factory()->create([
                'nama' => 'Penyelesaian Laporan Supervisi',
                'target_angka' => 2,
                'satuan' => 'laporan',
                'deskripsi' => 'Penyusunan dan penyerahan laporan supervisi mingguan.',
            ]),
            KpiMaster::factory()->create([
                'nama' => 'Eksekusi Pemeriksaan Lapangan',
                'target_angka' => 8,
                'satuan' => 'unit',
                'deskripsi' => 'Pemeriksaan dan penyelesaian pekerjaan lapangan sesuai target.',
            ]),
            KpiMaster::factory()->create([
                'nama' => 'Dokumentasi Bukti Pekerjaan',
                'target_angka' => 10,
                'satuan' => 'dokumen',
                'deskripsi' => 'Pengumpulan bukti pekerjaan harian dan dokumentasi pendukung.',
            ]),
            KpiMaster::factory()->create([
        ...
        foreach ($staffs as $staff) {
            ...
            foreach ($assignedKpis as $kpi) {
                UserKpiAssignment::factory()->create([
                    'user_id' => $staff->id,
                    'kpi_id' => $kpi->id,
                    'assigned_by' => $staff->is($staffLead) ? $admin->id : $staffLead->id,
                ]);
            }
            ...
            for ($date = clone $startDate; $date->lte($endDate); $date->addDay()) {
                if ($date->isWeekend() && $staff->is($staffSubordinate) && rand(0, 1) === 0) {
                    continue;
                }

                $statusChoice = $staff->is($staffLead) ? rand(1, 10) : rand(1, 12);
                $startHour = $staff->is($staffLead) ? 7 : 8;
                $endHour = $staff->is($staffLead) ? 16 : 17;
                $startTime = sprintf('%02d:%02d:00', rand($startHour, $startHour + 1), rand(0, 59));
                $endTime = sprintf('%02d:%02d:00', rand($endHour, $endHour + 1), rand(0, 59));
                $lokasiText = $staff->is($staffLead)
                    ? 'Kantor Operasional Pusat'
                    : 'Area Lapangan Sektor '.(string) rand(1, 4);
                $koordinat = $staff->is($staffLead)
                    ? ['lat' => -6.21000000, 'lng' => 106.84513000]
                    : ['lat' => -6.24000000 + (rand(0, 30) / 1000), 'lng' => 106.82000000 + (rand(0, 30) / 1000)];
                ...
                $finishedAt = $logbook->status === 'REJECTED'
                    ? null
                ...
                foreach ($assignedKpis as $index => $kpi) {
                    $progressRatio = match ($logbook->status) {
                        ...
                        'REJECTED' => 0.30 + ($index * 0.05),
                        default => 0.55 + ($index * 0.04),
                    };
                    $capaianAngka = round(min($kpi->target_angka * $progressRatio, $kpi->target_angka), 2);
                    ...
                    LogbookKpiDetail::factory()->create([
                        ...
                        'satuan' => $kpi->satuan,
                        'lampiran_file' => $index === 0 ? 'logbook-kpi-attachments/sample-proof-'.$staff->id.'-'.$date->format('Ymd').'.pdf' : null,
            ...
            $todayFinishedAt = Carbon::today()->setTime(rand(14, 17), rand(0, 59));

            $todayLogbook = Logbook::factory()->submitted()->create([
                'user_id' => $staff->id,
                'tanggal' => Carbon::today()->toDateString(),
                'start_kerja' => $staff->is($staffLead) ? '07:45:00' : '08:20:00',
                'end_kerja' => $staff->is($staffLead) ? '16:30:00' : '17:10:00',
                'lokasi' => $staff->is($staffLead) ? 'Kantor Operasional Pusat' : 'Area Lapangan Sektor 2',
                'lokasi_lat' => $staff->is($staffLead) ? -6.21000000 : -6.22350000,
                'lokasi_lng' => $staff->is($staffLead) ? 106.84513000 : 106.83320000,
            ]);
            ...
            foreach ($assignedKpis as $index => $kpi) {
                LogbookKpiDetail::factory()->create([
                    ...
                    'target_angka' => $kpi->target_angka,
                    'satuan' => $kpi->satuan,
                    'lampiran_file' => $index === 0 ? 'logbook-kpi-attachments/today-proof-'.$staff->id.'.pdf' : null,
                    'capaian_angka' => round($kpi->target_angka * (0.65 + ($index * 0.08)), 2),
