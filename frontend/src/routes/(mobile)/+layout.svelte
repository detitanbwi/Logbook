...
{#if !auth.isInitialized}
	<div class="flex min-h-screen flex-col bg-base-200">
		<header class="sticky top-0 z-30 flex items-center justify-between border-b border-base-300/70 bg-base-100/90 px-4 py-3 text-base-content shadow-sm backdrop-blur-md">
...
{:else if auth.isAuthenticated}
	<div class="flex min-h-screen flex-col bg-base-200">
		...
		<header class="sticky top-0 z-30 flex items-center justify-between border-b border-base-300/70 bg-base-100/90 px-4 py-3 text-base-content shadow-sm backdrop-blur-md">
			<div>
				<p class="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-base-content/55">Mobile Workspace</p>
				<h1 class="text-lg font-bold tracking-tight">Logbook</h1>
			</div>
			<div class="flex items-center gap-2">
				<span class="rounded-full border border-base-300/70 bg-base-100/70 px-2.5 py-1 text-xs font-medium text-base-content/80">{auth.user.current?.nama ?? ''}</span>
			</div>
		</header>
		...
		<nav class="fixed inset-x-3 bottom-3 z-40 flex items-center gap-1 rounded-2xl border border-base-300 bg-base-100 px-1 py-1.5 shadow-lg">
			{#each visibleTabs as tab (tab.href)}
				...
				>
					{#if tab.icon === 'home'}
						<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 transition-transform duration-200 group-active:scale-95 {active ? 'scale-105' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
